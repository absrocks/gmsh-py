...
for x in np.arange(x1+lc/4, x2+lc/4, lc/4):
    gmsh.model.geo.addPoint(x, 0, h_max, lc / 4, point_id)
    profile_pts.append(point_id)
    point_id += 1

# Base rectangle points (not added to profile_pts!)
gmsh.model.geo.addPoint(x0, 0, -2, lc, point_id)
base_left = point_id
point_id += 1

gmsh.model.geo.addPoint(x2, 0,-2, lc, point_id)
base_right = point_id
point_id += 1
...
h = 0.4
ov = gmsh.model.geo.extrude([(2, 1)], 0, 0, h, [2, 2], [0.5, 1])
gmsh.model.geo.synchronize()
